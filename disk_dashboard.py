...
class DiskDashboard:
    def __init__(self, root):
        self.root = root
        self.root.title("Disk Usage Dashboard")
        ...
        # Create usage label
        self.usage_label = ttk.Label(self.usage_frame, text="Disk Usage: 0%")
    ...
    def update_metrics(self):
        while True:
            self.usage = psutil.disk_usage('/').percent
            self.usage_label.config(text=f"Disk Usage: {self.usage}%")
            self.update_gauge()
            time.sleep(1)
